...
def book_slot(request, timeslot_id):
    ...
    # Check if the timeslot's room capacity has been exceeded
    if timeslot.has_available_seats():
        try:
        ...
        return JsonResponse({'success': False, 'message': 'This timeslot is fully booked.'})
...
def teacher_home(request):
    ...
    remaining_subject_credits = {}  # Dictionary to store remaining subject credits
    for subject in subjectss:
        ...
        remaining_subject_credits[subject.id] = remaining_credits

# Filter out subjects with remaining credits less than or equal to zero
    subjectss = [subject for subject in subjectss if remaining_subject_credits[subject.id] > 0]
    
    
    print (remaining_subject_credits) 
    ...
    context = {
        ...
        'remaining_subject_credits':remaining_subject_credits,
