...
class TimeSlot(models.Model):
    ...
    def has_available_seats(self):
        booked_seats = Booking.objects.filter(timeslot=self).count()
        return self.room.capacity > booked_seats
    # New method to check if the timeslot has available seats
    
    def update_availability(self):
        booked_seats = Booking.objects.filter(timeslot=self).count()
        self.booked_seats = booked_seats  # Update the booked seats count
